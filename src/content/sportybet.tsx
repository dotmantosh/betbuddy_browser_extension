...
import { ISportList, Tournament } from '../interfaces/ISportList';
import { IEvent, ITournament } from '../interfaces/IEvent'

const SportybetContent: React.FC<{ tournaments: ITournament[]; events: IEvent[] }> = ({ events, tournaments }) => {

  console.log('events in sportybet.tsx ', events)
  ...
  useEffect(() => {
    // Add event listeners to all .m-outcome elements
    const outcomes = document.querySelectorAll('.m-outcome');
    outcomes.forEach((outcome) => {
      outcome.addEventListener('click', handleOutcomeClick);
    });

    // Cleanup event listeners on unmount
    return () => {
      outcomes.forEach((outcome) => {
        outcome.removeEventListener('click', handleOutcomeClick);
      });
    };
  }, []);
  ...
  const handleOutcomeClick = (event: Event) => {
    ...
    // Find the parent .match-row
    const matchRow = clickedElement.closest('.m-table-row');
    if (!matchRow) return;

    // Get the gameId from .game-id
    const gameIdElement = matchRow.querySelector('.game-id');
    const gameId = gameIdElement?.textContent?.trim().replace('ID: ', '');

    // Get the odds from .m-outcome-odds
    const oddsElement = clickedElement.querySelector('.m-outcome-odds');
    const odds = oddsElement?.textContent?.trim();

    // Determine if the clicked element is in the first or second .m-market
    const twoMarkets = matchRow.querySelector('.two-markets');
    const markets = twoMarkets?.querySelectorAll('.m-market');
    let outcome = '';

    if (markets) {
      markets.forEach((market, index) => {
        if (market.contains(clickedElement)) {
          if (index === 0) {
            // First .m-market (home, draw, away)
            const outcomes = Array.from(market.querySelectorAll('.m-outcome'));
            const position = outcomes.indexOf(clickedElement);
            outcome = position === 0 ? 'Home' : position === 1 ? 'Draw' : 'Away';
          } else if (index === 1) {
            // Second .m-market (specifier)
            const specifierElement = market.querySelector('.specifier-select');
            const specifier = specifierElement?.textContent?.trim() || 'Unknown';

            // Determine if it's "Over" or "Under"
            const outcomes = Array.from(market.querySelectorAll('.m-outcome'));
            const position = outcomes.indexOf(clickedElement);
            const overUnder = position === 0 ? 'Over' : 'Under';

            // Combine specifier and over/under
            outcome = `${overUnder} ${specifier}`;
          }
        }
      });
    }

    // Log or store the selected outcome
    const selectedData = { gameId, odds, outcome };
    console.log('Selected Outcome:', selectedData);
